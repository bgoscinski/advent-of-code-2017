...
let cases: list((input, answer)) = [
  (String.concat(["0", "2", "7", "0"], ~sep="\t"), 5),
  (List.hd_exn(In_channel.read_lines("./inputs/day-06.txt")), 3156)
];
...
let solve = input => {
  ...
  let seen = Set.singleton(~comparator=IntMapCmp.comparator, mem);
  let rec realloc =
    fun
    | (mem, _, 0) => mem
    | (mem, bank, blocks) => {
  ...
  let rec pass = (seen, mem, step) => {
    ...
    if (Set.mem(seen, nextMem)) {
      step;
    } else {
      let seen = Set.add(seen, nextMem);
